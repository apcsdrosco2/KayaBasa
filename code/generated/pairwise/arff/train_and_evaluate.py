...
Setup:
  Row headers (TRAIN sets):
    Monolingual:  TGL, BCL, CEB
    Bilingual:    TGL+BCL, BCL+CEB, CEB+TGL
...
Outputs:
  results/
    ...
    results_detailed.txt       — full Weka output per experiment
...
def run_weka(train_arff: Path, test_arff: Path) -> dict:
    ...
    # Parse accuracy from Weka output
...
# ═══════════════════════════════════════════════════════════════════════════════
# Build "all languages" train ARFF files (stacking 3 train splits)
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_all_languages_arff():
    """Stack all 3 language train ARFFs into combined ARFF files."""
    for feat in FEATURE_SETS:
        # Read all 3 train ARFF files
        all_lines_header = []
        all_lines_data = []
        header_done = False
...
# ─── 2. Bilingual: Train on pair, test on ALL languages ──────────────────────
...
for lang1, lang2 in PAIRS:
    ...
    for feat in FEATURE_SETS:
        # Find the train file (may be in reverse order)
...
# ─── 3. All languages: Train on all 3, test on each ──────────────────────────
